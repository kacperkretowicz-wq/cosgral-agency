...
async def audit_mode(name: str) -> dict:
    ...
    async with async_playwright() as p:
        ...
        await page.evaluate(f"""() => {{
            localStorage.setItem('effectMode', '{name}');
...
async def main():
    ...
    modes = [
        "classic", "liquid", "spark", "kinetic", "glitch", "ink",
        "particles", "slanted", "strobe", "matrix", "vortex",
        "slingshot", "decode", "zoom", "flip"
    ]
    ...
    if not has_issues:
        print("\nALL 15 MODES PASSED AUDIT WITH NO ERRORS OR WARNINGS!")

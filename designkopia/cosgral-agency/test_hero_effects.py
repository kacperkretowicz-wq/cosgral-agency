...
async def run_test(name: str, prefers_reduced_motion: str, effect_mode: str) -> dict:
    print(f"Running test: {name} (reduced-motion: {prefers_reduced_motion}, mode: {effect_mode})")
    ...
    async with async_playwright() as p:
        ...
        await page.evaluate(f"""() => {{
            localStorage.setItem('effectMode', '{effect_mode}');
        ...
        await page.evaluate("""() => {
            ...
            const st = ScrollTrigger.getAll().find(s => s.trigger === wrap && s.end === 8100);
            if (st) {
...
async def main():
    ...
    results["reduced_motion_reduce_glitch"] = await run_test(
        name="reduce_motion_reduce_glitch",
        prefers_reduced_motion="reduce",
        effect_mode="glitch"
    )
    
    results["reduced_motion_no_preference_glitch"] = await run_test(
        name="reduced_motion_no_preference_glitch",
    ...
    t1 = results["reduced_motion_reduce_glitch"]
    print(f"Test 1 (reduced_motion_reduce_glitch):")
    ...
    t2 = results["reduced_motion_no_preference_glitch"]
    print(f"\nTest 2 (reduced_motion_no_preference_glitch):")

"""Build original, illustrative product stills for the TelForceOne case studies in pure B&W / monochrome aesthetic."""
...
FILLS = {
    "#F5F5F1": "#0D1013", "#FCFCF9": "#13171B", "#F0F1ED": "#101418",
    "#FFFFFF": "#181D22", "#FCFCFA": "#15191D", "#F5F6F2": "#1A1F25",
    "#E8EDE9": "#12161A", "#E9EAE5": "#111519", "#E9EEF4": "#161B20",
    "#E8EEE8": "#15191E", "#E4ECE6": "#1B2127",
}
INK = {
    "#20242A": "#F0F3F6", "#22272B": "#F0F3F6", "#242B2F": "#F4F7F9",
    "#252C2E": "#F2F5F7", "#25292A": "#EDF0F3", "#2D3437": "#E2E7EC",
    "#30383A": "#E2E7EC", "#343C3D": "#D5DCE2", "#394243": "#D5DCE2",
    "#3D4547": "#CBD3D9", "#515A5A": "#B2BAC2", "#808882": "#8A949E",
    "#838986": "#8A949E", "#838A84": "#8A949E", "#89908B": "#808A94",
    "#8B918C": "#808A94", "#929A93": "#808A94", "#949A95": "#808A94",
    "#969B96": "#808A94", "#9AA09A": "#808A94", "#7D827E": "#808A94",
    "#858884": "#808A94", "#8C918D": "#808A94", "#B5B3AD": "#9AA4AE",
    "#E8E5DF": "#F0F3F6", "#69756E": "#9CA6B0", "#47614E": "#F0F3F6",
}
LINES = {
    "#D9DBD7": "#252C33", "#DADDD8": "#252C33", "#D7DAD4": "#252C33",
    "#E1E4DF": "#2A323A", "#E2E4DF": "#2A323A", "#E7E9E4": "#242A32",
    "#EBEDE9": "#242A32", "#C9CECA": "#3A4550", "#313438": "#222830",
}
...
def pill(x, y, w, label, fill="#161B20", color="#FFFFFF"):
    return rect(x, y, w, 28, fill, 14, "#3A4550") + text(x + 13, y + 19, label, 13, color, 700)


def chrome(index, category, accent="#FFFFFF"):
    return f'''<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1280 720" role="img">
<defs>
  <filter id="shadow" x="-20%" y="-30%" width="140%" height="160%"><feDropShadow dx="0" dy="22" stdDeviation="22" flood-color="#000" flood-opacity=".5"/></filter>
  <clipPath id="screen"><rect x="70" y="108" width="1140" height="542" rx="10"/></clipPath>
</defs>
<rect width="1280" height="720" fill="#080A0C"/>
<path d="M0 75H1280M0 683H1280" stroke="#222830"/>
...
<g filter="url(#shadow)"><rect x="70" y="108" width="1140" height="542" rx="10" fill="#0D1013" stroke="#252C33"/></g>
...
<circle cx="1174" cy="134" r="13" fill="#252C33" stroke="#FFFFFF" stroke-width="1.5"/>{text(1169,139,'TF',10,'#FFFFFF',700)}
'''
...
def nav(active, accent="#FFFFFF"):
    ...
    for i,(label,y) in enumerate([("Przegląd",268),("Klienci",315),("Mapa tras",362),("Etykiety",409),("Zapasy",456),("Raporty",503)]):
        if label==active:
            s += rect(81,y-24,147,38,"#FFFFFF",6) + rect(81,y-24,3,38,"#FFFFFF",1)
        s += text(102,y,label,15,"#FFFFFF" if label==active else "#838986",700 if label==active else 400)
...
def crm():
    ...
    for d in ["M270 283C370 308 415 278 500 295S688 327 876 292", "M272 370C390 347 430 376 530 357S730 329 876 352", "M270 473C385 449 476 490 586 465S765 434 876 458", "M344 254C360 355 334 457 370 604", "M511 254C483 344 525 478 512 604", "M703 254C730 370 689 495 721 604", "M825 254C795 364 830 472 809 604"]:
        s += f'<path d="{d}" fill="none" stroke="#252C33" stroke-width="12" stroke-linecap="round" opacity=".95"/>'
        s += f'<path d="{d}" fill="none" stroke="#353E47" stroke-width="1.5" opacity=".7"/>'
    s += rect(290,272,113,31,"#FFFFFF",5) + text(302,293,"KRAKÓW",12,"#69756E",700,1)
    s += '<path d="M360 515C430 480 440 440 511 420S612 390 645 347S752 355 810 307" fill="none" stroke="#101418" stroke-width="11" stroke-linecap="round"/>'
    s += '<path d="M360 515C430 480 440 440 511 420S612 390 645 347S752 355 810 307" fill="none" stroke="#E2E7EC" stroke-width="4" stroke-linecap="round" stroke-dasharray="3 3"/>'
    for x,y,n in [(360,515,"1"),(511,420,"2"),(645,347,"3"),(810,307,"4")]:
        s += f'<circle cx="{x}" cy="{y}" r="16" fill="#F0F3F6" stroke="#101418" stroke-width="3"/>' + text(x-4,y+5,n,14,"#0D1013",700)
    ...
    for y,time,title,sub in [(374,"09:30","Sklep K1","Kraków · centrum"),(449,"11:15","Partner 024","Wieliczka"),(524,"14:00","Punkt 018","Bochnia")]:
        s += f'<circle cx="930" cy="{y-4}" r="5" fill="#E2E7EC"/>' + text(947,y,time,12,"#FFFFFF",700) + text(947,y+23,title,16,"#2D3437",700) + text(947,y+41,sub,12,"#89908B")
    s += text(919,581,"OTWÓRZ KARTĘ KLIENTA →",11,"#FFFFFF",700,1)
    return s + end()
...
def barcode(data, x=733, y=356, unit=2.8, height=160):
    ...
    for char in "*"+data+"*":
        for i,c in enumerate(CODE39[char]):
            ...
            if i%2==0: parts.append(rect(round(cursor,2),y,round(width,2),height,"#F0F3F6"))
            cursor+=width
        cursor+=unit
    return ''.join(parts)
...
def code39():
    accent = "#FFFFFF"
    s = chrome("02", "CODE 39", accent) + nav("Etykiety", accent)
    ...
    # Clean white action button with crisp dark typography
    s += rect(296,540,352,39,"#F0F3F6",5) + text(363,565,"GENERUJ ETYKIETĘ ↗",14,"#0D1013",700,1)
    s += rect(701,268,480,328,"#E9EAE5",8)
    s += text(727,301,"PODGLĄD DO DRUKU",12,"#8B918C",700,1.1)
    s += rect(725,323,430,240,"#0D1013",5,"#3A4550")
    s += text(747,346,"TELFORCEONE",13,"#A0A8B0",700,1.2)
    s += barcode("TF1-2026",746,367,2.35,131)
    s += text(846,528,"*TF1-2026*",18,"#F0F3F6",700,3)
...
def forecast():
    ...
    # Monochrome area and crisp forecast lines
    s += '<path d="M300 508 L354 491 L408 501 L462 480 L516 487 L570 467 L624 481 L678 458 L732 450 L786 432 L839 444 L839 576 L300 576Z" fill="#182028"/>'
    s += '<path d="M300 508 L354 491 L408 501 L462 480 L516 487 L570 467 L624 481 L678 458 L732 450 L786 432 L839 444" fill="none" stroke="#FFFFFF" stroke-width="3.5" stroke-linecap="round" stroke-linejoin="round"/>'
    s += '<path d="M300 546 L354 543 L408 534 L462 530 L516 524 L570 510 L624 507 L678 492 L732 482 L786 478 L839 470" fill="none" stroke="#8A949E" stroke-width="2" stroke-dasharray="5 5"/>'
    ...
    for y,sku,desc,status in [(456,"TF-0428","zapas na 6 dni","PILNE"),(507,"TF-1081","zapas na 9 dni","UWAGA"),(558,"TF-2204","zapas na 12 dni","OK")]:
        s += line(919,y+11,1159,y+11,"#EBEDE9") + text(920,y,sku,15,"#343C3D",700) + text(920,y+19,desc,11,"#929A93") + text(1103,y,status,10,"#FFFFFF" if status=="PILNE" else "#A0A8B0",700,1)
    return s + end()
...
if __name__ == "__main__":
    ...
    for name, build in [("crm", crm), ("code39", code39), ("forecast", forecast)]:
        artwork = build()
        out_file = OUT / f"telforceone-{name}.svg"
        out_file.write_text(artwork, encoding="utf-8")
        print(f"Generated {name} -> {out_file} ({out_file.stat().st_size} bytes)")
